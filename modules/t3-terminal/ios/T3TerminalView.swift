...
public final class T3TerminalView: ExpoView, UITextFieldDelegate {
  ...
  let onViewportScroll = EventDispatcher()
  ...
  private func handleViewportPan(_ gesture: UIPanGestureRecognizer) {
    ...
    case .changed:
      ...
      if externalScroll {
        // UIKit translation is negative for an upward swipe. The shared event contract uses
        // positive rows for moving toward the terminal bottom, matching Android.
        onViewportScroll(["rows": -verticalSteps])
      } else {
